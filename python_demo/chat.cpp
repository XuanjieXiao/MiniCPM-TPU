...
public:
  ...
  std::mt19937 sgen;
  MiniCPM() : sgen(std::random_device()()){};
...
void MiniCPM::init(const std::vector<int> &devices, std::string model_path) {
...
void MiniCPM::head_launch(const bm_net_info_t *net, bm_device_mem_t &logits_mem) {
  std::vector<bm_tensor_t> in_tensors(net->input_num);
  std::vector<bm_tensor_t> out_tensors(net->output_num);
...
int MiniCPM::greedy_search(const bm_net_info_t *net, bm_device_mem_t &logits_mem) {
...
int MiniCPM::penalty_sample(const bm_net_info_t *net, bm_device_mem_t &logits_mem) {
...
std::vector<int> MiniCPM::generate(std::vector<int> &history_tokens, int EOS) {
...
PYBIND11_MODULE(chat, m) {
    pybind11::class_<MiniCPM>(m, "MiniCPM")
        .def(pybind11::init<>())
        .def("init", &MiniCPM::init)
        .def("forward_first", &MiniCPM::forward_first)
        .def("forward_next", &MiniCPM::forward_next)
        .def("generate", &MiniCPM::generate)
        .def("deinit", &MiniCPM::deinit)
        .def_readwrite("SEQLEN", &MiniCPM::SEQLEN) // read SEQLEN in pipeline.py
        .def_readwrite("token_length", &MiniCPM::token_length)
        .def_readwrite("temperature", &MiniCPM::temperature)
        .def_readwrite("top_p", &MiniCPM::top_p)
        .def_readwrite("repeat_penalty", &MiniCPM::repeat_penalty)
        .def_readwrite("repeat_last_n", &MiniCPM::repeat_last_n)
        .def_readwrite("max_new_tokens", &MiniCPM::max_new_tokens)
        .def_readwrite("generation_mode", &MiniCPM::generation_mode)
        .def_readwrite("prompt_mode", &MiniCPM::prompt_mode);
}
